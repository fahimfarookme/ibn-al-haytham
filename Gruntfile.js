...
module.exports = function(grunt) {
   ...
   // build properties
   var config = (function() {
      var build = grunt.file.readJSON(".ibn-al-haythamrc");
      if (!build) {
         grunt.fail.fatal([".ibn-al-haytham doesn't exists"]);
      }
      ...
      if (!build.optimize) {
         grunt.log.error([".ibn-battuta doesn't define optimize. Command line options or defaults will be used."]);
      ...
      if (!(build.src && build.test && build.dist && build.temp)) {
         grunt.fail.fatal([".ibn-battuta doesn't define mandatory directories: src, test, dist, temp"]);
      }
   ...
   grunt.initConfig({
      ...
      copy: {
         ...
         // copy module css/ sass files
         // from: src, to: .tmp
         // in order to other css related tasks (i.e. autoprefixer) can work on them
         // without modifying actual source css
         css_src_temp: {
            expand: true,
            dot: true,
            cwd: "<%= config.src %>",
      ...
      // from src sass --> to .tmp css
      sass: {
         options: {
            loadPath: "<%= config.bower.directory %>",
            require: ["susy"]
         },
         src_temp: {
            files: [{
      ...
      // TODO check whether conflicts with our compass sass
      // the need of copying styles to .tmp and usemin->source=.tmp comes because of this only.
      // once autoprefixed, we need concat task to use autoprefixed css.
      // we should not autoprefix on our source itself
      postcss: {
         options: {
            map: true, // inline sourcemaps
            processors: [
               // add vendor prefixes for following browsers
               require("autoprefixer")({
                  browsers: ["Explorer > 7", "last 2 Chrome versions", "last 2 Firefox versions", "Safari > 4"]
               })
            ]
         },
         target: {
      ...
      // minify css files
      // usemin auto generates cssmin:generated task which we'll use for non_modules
      // the cssmin:modules task below will be used for minifying module css.
      // no requirejs is used because requirejs requires a master css file with imports to other css files
      cssmin: {
         minify: {
            files: [{
               expand: true,
         ...
         concat_minify: {
            files: {
               "<%= config.dist %>/<%= config.optimize.css.file %>": ["<%= config.temp %>/**/*.css"]
            }
         }
   ...
   grunt.registerTask("package", function() {
      ...
      tasks = tasks.concat(["copy:css_src_temp", "sass:src_temp", "postcss"]);
      if (config.optimize.css.concat) { // concat (& minify). concat without minify is not allowed - see config() function.
         tasks.push("cssmin:concat_minify");
      } else if (config.optimize.css.minify) { // minify, no concat
         tasks.push("cssmin:minify");
      } else { // (!config.optimize.css.concat && !config.optimize.css.minify)
         tasks.push("copy:css_temp_dist");
      }
